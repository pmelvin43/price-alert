        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            if (isset($_POST["username"]) && isset($_POST["password"])) {
                ...
                if ($result && mysqli_num_rows($result) > 0) {
                    echo "<script>alert('User has a valid account.'); window.location.href = 'home.php';</script>";
                } else {
                    echo "<script>alert('Invalid username and/or password.'); window.location.href = 'home.php';</script>";
                }
            } else {
